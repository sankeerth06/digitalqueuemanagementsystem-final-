...
export function StaffDashboard() {
  ...
  const { data: settings } = useQuery({
    queryKey: ['settings'],
    queryFn: async () =>
      (await api.get('/settings')).data.data.settings as SystemSettings,
  });
  ...
  const preparing =
    queue?.filter((t) => t.status === 'preparing') ?? [];

  const ready =
    queue?.filter((t) => t.status === 'ready') ?? [];
  ...
  const togglePause = async () => {
    if (settings?.queuePaused) {
      ...
      await api.post('/settings/pause', {
        reason: 'Staff paused the queue',
      });
    }
    ...
    <div className="mx-auto max-w-6xl space-y-6">
      ...
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-sm text-ink-600/60 dark:text-white/50">
            Manage counters and move the queue forward.
          </p>
        </div>
        ...
        <div className="flex items-center gap-3">
          ...
          {/* Call next */}
          <Button
            onClick={() => callNext.mutate(counter)}
        ...
        <section>
          ...
          {!isLoading && !waiting.length && (
            <EmptyState
              ...
              description="New tokens will appear here as students book them."
            />
          )}
        ...
        <section>
          ...
          {!preparing.length && (
            <EmptyState
              ...
              description="Call the next token to start preparing an order."
            />
          )}
      ...
      <section>
        ...
        <h2 className="mb-3 flex items-center gap-2 text-sm font-semibold text-ink-600/70 dark:text-white/50">
          ...
          Skipped tokens ({/* count handled by list */})
        </h2>
...
function SkippedList({
  ...
  if (!skipped?.length) {
    return (
      <p className="text-sm text-ink-600/50 dark:text-white/40">
        No skipped tokens right now.
      </p>
    );
  }
